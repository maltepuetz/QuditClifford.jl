"""
    ...
    StabilizerTableau(d::Int, tableau::AbstractMatrix{<:Integer}; m::Union{Int,Nothing}=nothing, storephase::Union{Bool,Nothing}=nothing, check::Bool=true)
...
Both properties are contract: the raw-matrix constructor enforces them (see `check`), the presets
satisfy them by construction, and `measure!` and [`canonicalize!`](@ref) preserve them. That is what
lets [`is_pure`](@ref) decide on `m == n` alone.
...
function _build_stabilizer_tableau(
    ...
    # A preset filler satisfies the generator contract by definition, so the
    # (d, n) path never pays for the check. Validating the built tableau rather
    # than the raw matrix lets the check borrow `workspace`.
    check && !preset && _validate_generators!(out)
