...
# say. Neither was enforced, so an invalid matrix built a tableau that every
# later operation then believed -- `measure!` decomposing against a basis that
# does not span what it claims, with `is_pure` the only thing that ever noticed.
#
# Checking here is what promotes the contract to an invariant, because
# construction is the only door into it: presets satisfy it by definition, and
# `measure!` and `canonicalize!` both preserve it. That is what lets `is_pure`
# rest on `m == n`.
#
